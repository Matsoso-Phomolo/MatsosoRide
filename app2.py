...
def repair_seed_data():
    """Fix known seeded records in existing databases."""
    global _seed_repairs_done
    if _seed_repairs_done:
        return
    try:
        m(
            "UPDATE Drivers SET first_name=%s, last_name=%s, password_hash=%s "
            "WHERE phone_number=%s AND (first_name<>%s OR last_name<>%s OR password_hash<>%s)",
            ("Rets'elisitsoe", "Tau",
             "4c13160c02ae787e3f688d032145f36e503570f1f99b5ea1d048cb8d861829e0",
             "+26658200002", "Rets'elisitsoe", "Tau",
             "4c13160c02ae787e3f688d032145f36e503570f1f99b5ea1d048cb8d861829e0")
        )
        m(
            "UPDATE Drivers SET password_hash=%s WHERE phone_number=%s AND password_hash<>%s",
            ("00384db782b0f9fe1da2e884b438db3d9183470ed99eb7a9a8ff704965561a7e",
             "+26658200001",
             "00384db782b0f9fe1da2e884b438db3d9183470ed99eb7a9a8ff704965561a7e")
        )
        m(
            "UPDATE Drivers SET password_hash=%s WHERE phone_number=%s AND password_hash<>%s",
            ("ddbb9af440b078a0bbf7407b805bba01779e8b8d9141223429f2a16aee29e967",
             "+26658200003",
             "ddbb9af440b078a0bbf7407b805bba01779e8b8d9141223429f2a16aee29e967")
        )
    except Exception:
        return
    _seed_repairs_done = True
...
def admin_rides():
    ...
    args = []
    if search:    sql += " AND (passenger_name LIKE %s OR passenger_phone LIKE %s OR COALESCE(driver_name,'') LIKE %s OR pickup_name LIKE %s OR dropoff_name LIKE %s)"; args += [f'%{search}%']*5
    if status:    sql += " AND ride_status=%s"; args.append(status)
    if loc_id:    sql += " AND (pickup_location_id=%s OR dropoff_location_id=%s)"; args += [loc_id]*2
    if date_from: sql += " AND DATE(requested_at)>=%s"; args.append(date_from)
    if date_to:   sql += " AND DATE(requested_at)<=%s"; args.append(date_to)
...
def admin_locations():
    ...
    sql    = "SELECT *, total_rides AS ride_count FROM vw_location_demand WHERE 1=1"
    args   = []
    if search: sql += " AND location_name LIKE %s"; args.append(f'%{search}%')
    if zone:   sql += " AND area_zone=%s"; args.append(zone)
    sql += " ORDER BY area_zone, location_name"
